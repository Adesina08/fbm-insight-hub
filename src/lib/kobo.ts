...
function interpretDifference(label: string, beta: number | null, userAverage: number | null, nonUserAverage: number | null): string {
...
function computeModelSummary(submissions: KoboSubmission[], quadrants: QuadrantInsight[]): ModelSummary[] {
  ...
  return [
    ...
    },
    {
      ...
      helper: dominantQuadrant ? `${dominantQuadrant.count} respondents (${dominantQuadrant.percentage.toFixed(0)}%)` : "Not enough data",
    },
  ];
}
