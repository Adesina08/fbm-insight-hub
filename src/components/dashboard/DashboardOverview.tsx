...
const DashboardOverview = ({
  ...
  const insightSections = useMemo<InsightSection[]>(() => {
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                n = {formatNumber(summary.count, { maximumFractionDigits: 0 })}
              </span>
            </div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5 print:grid-cols-3">
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-3xl font-semibold text-foreground">
                    {formatDecimal(summary.age.mean, 1)}
                    <span className="ml-1 text-base font-medium text-muted-foreground">yrs</span>
                  </p>
                  <p>Median {formatDecimal(summary.age.median, 0)} yrs · SD {formatDecimal(summary.age.sd, 1)}</p>
                  <p className="text-xs text-muted-foreground">
                    Valid responses: {formatNumber(summary.age.count, { maximumFractionDigits: 0 })}
                  </p>
                </CardContent>
              </Card>
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-2xl font-semibold text-foreground">{summary.marital.label ?? "n/a"}</p>
                  <p>
                    {summary.marital.share != null
                      ? `${formatPercentage(summary.marital.share * 100)} of respondents`
                      : "No responses"}
                  </p>
                  {summary.marital.unknownCount > 0 ? (
                    <p className="text-xs text-muted-foreground">
                      Unknown: {formatNumber(summary.marital.unknownCount, { maximumFractionDigits: 0 })}
                    </p>
                  ) : null}
                </CardContent>
              </Card>
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-2xl font-semibold text-foreground">{summary.education.label ?? "n/a"}</p>
                  <p>
                    {summary.education.share != null
                      ? `${formatPercentage(summary.education.share * 100)} of respondents`
                      : "No responses"}
                  </p>
                  {summary.education.unknownCount > 0 ? (
                    <p className="text-xs text-muted-foreground">
                      Unknown: {formatNumber(summary.education.unknownCount, { maximumFractionDigits: 0 })}
                    </p>
                  ) : null}
                </CardContent>
              </Card>
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-2xl font-semibold text-foreground">{summary.location.label ?? "n/a"}</p>
                  <p>
                    {summary.location.share != null
                      ? `${formatPercentage(summary.location.share * 100)} of respondents`
                      : "No responses"}
                  </p>
                  {summary.location.unknownCount > 0 ? (
                    <p className="text-xs text-muted-foreground">
                      Unknown: {formatNumber(summary.location.unknownCount, { maximumFractionDigits: 0 })}
                    </p>
                  ) : null}
                </CardContent>
              </Card>
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-3xl font-semibold text-foreground">
                    {formatDecimal(summary.parity.mean, 1)}
                    <span className="ml-1 text-base font-medium text-muted-foreground">children</span>
                  </p>
                  <p>Median {formatDecimal(summary.parity.median, 1)} · SD {formatDecimal(summary.parity.sd, 1)}</p>
                  <p className="text-xs text-muted-foreground">
                    Valid responses: {formatNumber(summary.parity.count, { maximumFractionDigits: 0 })}
                  </p>
                </CardContent>
              </Card>
            </div>
    ...
    if (motivationSummaries) {
      const summaries = motivationSummaries.filter((item) => item.summary.count > 0);
      if (summaries.length > 0) {
        sections.push({
          key: "motivation",
          label: "Motivation sub-domains",
          render: () => (
            <div className="space-y-4">
    ...
    if (abilitySummaries) {
      const summaries = abilitySummaries.filter((item) => item.summary.count > 0);
      if (summaries.length > 0) {
        sections.push({
          key: "ability",
          label: "Ability sub-domains",
          render: () => (
            <div className="space-y-4">
    ...
    if (normSummaries && (normSummaries.descriptive.count > 0 || normSummaries.injunctive.count > 0)) {
      const norms = normSummaries;
      sections.push({
        key: "norms",
        label: "Social norms",
        render: () => (
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 print:grid-cols-2">
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-2xl font-semibold text-foreground">
                    {norms.descriptive.mean != null ? norms.descriptive.mean.toFixed(2) : "n/a"}
                    {norms.descriptive.mean != null ? (
                      <span className="ml-1 text-sm font-medium text-muted-foreground">/ 5</span>
                    ) : null}
                  </p>
                  <p>Median {formatDecimal(norms.descriptive.median, 2)} · SD {formatDecimal(norms.descriptive.sd, 2)}</p>
                  <p>
                    High agreement (4–5):
                    {" "}
                    {norms.descriptive.positiveShare != null
                      ? formatPercentage(norms.descriptive.positiveShare * 100)
                      : "n/a"}
                  </p>
                  {norms.descriptive.topValue != null ? (
                    <p>
                      Most common rating: {norms.descriptive.topValue}
                      {norms.descriptive.topShare != null
                        ? ` (${formatPercentage(norms.descriptive.topShare * 100)})`
                        : ""}
                    </p>
                  ) : null}
                  <p className="text-xs text-muted-foreground">
                    Valid responses: {formatNumber(norms.descriptive.count, { maximumFractionDigits: 0 })}
                  </p>
                </CardContent>
              </Card>
              <Card className="relative border-0 bg-card/50 backdrop-blur-sm">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p className="text-2xl font-semibold text-foreground">
                    {norms.injunctive.mean != null ? norms.injunctive.mean.toFixed(2) : "n/a"}
                    {norms.injunctive.mean != null ? (
                      <span className="ml-1 text-sm font-medium text-muted-foreground">/ 5</span>
                    ) : null}
                  </p>
                  <p>Median {formatDecimal(norms.injunctive.median, 2)} · SD {formatDecimal(norms.injunctive.sd, 2)}</p>
                  <p>
                    High approval (4–5):
                    {" "}
                    {norms.injunctive.positiveShare != null
                      ? formatPercentage(norms.injunctive.positiveShare * 100)
                      : "n/a"}
                  </p>
                  {norms.injunctive.topValue != null ? (
                    <p>
                      Most common rating: {norms.injunctive.topValue}
                      {norms.injunctive.topShare != null
                        ? ` (${formatPercentage(norms.injunctive.topShare * 100)})`
                        : ""}
                    </p>
                  ) : null}
                  <p className="text-xs text-muted-foreground">
                    Valid responses: {formatNumber(norms.injunctive.count, { maximumFractionDigits: 0 })}
                  </p>
                </CardContent>
              </Card>
            </div>
  ...
  useEffect(() => {
    setActiveInsightIndex((current) => {
      ...
      const maxIndex = insightSections.length - 1;
        ...
        <>
          <div className="relative print:hidden">
            {insightSections.length > 1 ? (
              <>
                <Button
                  ...
                  variant="default"
                  size="icon"
                  className="absolute left-0 top-1/2 z-10 h-12 w-12 -translate-x-1/2 -translate-y-1/2 rounded-full shadow-2xl ring-2 ring-primary/30 transition-all hover:shadow-[0_10px_30px_rgba(59,130,246,0.35)] focus-visible:ring-4 focus-visible:ring-primary/40 sm:h-14 sm:w-14"
                  onClick={() => setActiveInsightIndex((current) => Math.max(0, current - 1))}
                  disabled={activeInsightIndex === 0}
                  aria-label="View previous insight section"
                >
                ...
                <Button
                  ...
                  variant="default"
                  size="icon"
                  className="absolute right-0 top-1/2 z-10 h-12 w-12 translate-x-1/2 -translate-y-1/2 rounded-full shadow-2xl ring-2 ring-primary/30 transition-all hover:shadow-[0_10px_30px_rgba(59,130,246,0.35)] focus-visible:ring-4 focus-visible:ring-primary/40 sm:h-14 sm:w-14"
                  onClick={() =>
                    setActiveInsightIndex((current) => Math.min(insightSections.length - 1, current + 1))
                  }
                  disabled={activeInsightIndex >= insightSections.length - 1}
                  aria-label="View next insight section"
                >
            ...
            <div className="px-2 sm:px-10">
              {insightSections[activeInsightIndex]?.render()}
            </div>
            {insightSections.length > 1 ? (
              <div className="mt-6 flex flex-col items-center gap-4">
                <span className="text-sm font-medium text-muted-foreground">
                  Page {activeInsightIndex + 1} of {insightSections.length}
                </span>
          ...
          <div className="hidden print:flex flex-col space-y-6">
            {insightSections.map((section) => (
              <div key={`print-${section.key}`}>{section.render()}</div>
            ))}
          </div>
        </>
      ) : null}
