...
const DashboardOverview = ({ stats, quadrants, lastUpdated, isLoading = false, error, onRetry }: DashboardOverviewProps) => {
  ...
  if (!stats || !quadrants) {
    return <ErrorState message="No Google Sheets responses are available yet." onRetry={onRetry} />;
  }
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm text-muted-foreground">
            Last Sheets sync: <span className="font-medium text-foreground">{lastUpdatedLabel(lastUpdated)}</span>
          </p>
        ...
        <CardContent>
          {hasQuadrantData ? (
            ...
            <div className="text-sm text-muted-foreground">
              Google Sheets submissions do not yet contain motivation and ability scores. Once they are provided, the quadrant
